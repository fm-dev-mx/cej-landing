...
export function useCheckout() {
    ...
    const processOrder = async (
        ...
        try {
            ...
            // 1. Generate deduplication key (UUID)
            ...
            const orderPayload: OrderPayload = {
                ...
                items: cart.map((i) => ({
                    ...
                    // Include additives in the JSONB payload
            ...
            trackLead({
                ...
                event_id: fbEventId, // Critical for deduplication
            });

            // 3. Send to server (with Event ID)
            const result = await submitLead({
                name: customer.name,
                phone: customer.phone,
                quote: orderPayload as any, // Mantiene la lógica actual
                visitor_id: identity?.visitorId,
                utm_source: identity?.utm_source,
                utm_medium: identity?.utm_medium,
                fb_event_id: fbEventId, // Passed to server for CAPI
            ...
            // Secondary event (no deduplication needed usually)
