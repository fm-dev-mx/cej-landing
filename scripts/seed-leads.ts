...
async function seed() {
    ...
    try {
        ...
        console.warn(`✅ Successfully seeded ${data?.length} sample leads.`);
        console.warn("IDs inserted:", data?.map(d => d.id).join(", "));
