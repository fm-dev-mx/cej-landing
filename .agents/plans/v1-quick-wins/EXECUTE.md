...
- **Step 02:** Touch `app/(admin)/dashboard/layout.tsx` (redirect fix), `app/(public)/login/` (verify/create), `app/(admin)/dashboard/page.tsx` (add new order CTA), `app/(admin)/dashboard/new/` (new page), `app/actions/createAdminOrder.ts` (new). Route protection is already handled by `proxy.ts` — no `middleware.ts` needed. Do **not** touch `app/actions/submitLead.ts` — admin orders use a dedicated action.
